...
void WarpSpace(float h2, inout vec3 eyevec, inout vec3 raypos)
{
    vec3 origin = vec3(0.0, 0.0, 0.0);

    float r = distance(raypos, origin);
    vec3 r_norm = normalize(origin - raypos);
    float warpAmount = 5.0;

    eyevec = normalize(eyevec + h2/pow(r,5.0)*r_norm/30.);
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec3 eyevec = normalize(vec3((uveye * 2.0 - 1.0) * vec2(aspect, 1.0), 2.0));
    ...
    vec3 origin = vec3(0.0, 0.0, 0.0);
    float h = length(cross(origin - raypos,eyevec));

    for (int i = 0; i < ITERATIONS; i++)
    {        
        WarpSpace(h*h, eyevec, raypos);
